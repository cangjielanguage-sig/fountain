...
| 操作 | 中位数 | 误差 | 说明 |
|------|-------:|-----|------|
| `add` | 31.5 ms | ±4.9% | 1000 次顺序写入 |
| `get` | 57.6 ms | ±9.4% | 1000 次随机读取（先写入后读取） |
| `remove` | 82.5 ms | ±5.8% | 1000 次删除 |
| `prefix` | 34.8 ms | ±12.1% | 500 条前缀遍历 |
| `ttl` | 176.4 ms | ±11.3% | 1000 次 TTL 更新 |
| `addWithExpire` | 94.5 ms | ±13.7% | 1000 次带 TTL 写入 |

#### 并发

| 操作 | 中位数 | 误差 | 说明 |
|------|-------:|-----|------|
| `concurrentAdd` (4线程) | 159.8 ms | ±26.1% | 4 线程各写入 250 条 |
| `mixed` (5线程) | 0.602 s | ±4.9% | add/get/remove/prefix/ttl 混合运行 500ms |

**注意**：`benchConcurrentGet` 因 OOM 未完成。运行基准测试建议增大堆：
```bash

...
## ✅ Step 2: IO 层 — 使用 std.fs.File 同步 IO

**文件**: `WAL.cj`、`SSTable.cj`、`Store.cj`
**包**: `fountain::f_store`

### 开发内容
WAL 和 SSTable 直接使用 `std.fs.File`（实现 `IOStream & Resource`）进行 IO 操作。
不再使用 IOURingStream，f_store 不依赖 fountain::f_io。

### 设计说明
- `std.fs.File` 已实现 `IOStream & Resource`，直接使用即可
- Linux 和非 Linux 统一使用 File 同步 IO，简化跨平台处理
- `flush()` 通过 `file.flush()` 确保数据落盘
...
2. **编码格式**（小端序）:
   ...
   ```
   - value_len = -1 表示 tombstone，>=0 为正常 value（含空数组）
...
| 差异点 | 原方案 | 新方案 |
|--------|--------|--------|
| IO 层 | StoreIOStream + IOUringStoreIOStream + FileStoreIOStream | std.fs.File 同步 IO（统一跨平台） |
| WAL 写锁 | Mutex writeLock | 无锁（File 同步写入无竞争） |
| WAL IO | IoUringLockFree + IoUring + 收割线程 | std.fs.File 同步 IO |
...
| 平台支持 | 仅 Linux | Linux + 非Linux 统一 std.fs.File |
| CQE 收割 | 自定义收割线程 | 无（不使用 io_uring） |
| SSTable IO | 自定义 IOUringStoreIOStream | std.fs.File 同步 IO |
| fsync | ring.getSQE + ioUringPrepFsync | file.flush()（标准 fsync） |

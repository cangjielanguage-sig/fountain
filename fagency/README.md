...
--对于search，llm_type是'search'；llm_name 是具体的搜索名称，智谱是'glm-search_std' 'glm-search_pro' 'glm-search_pro_sogou' 'glm-search_pro_quark'，其它都一样
```
## 配置知识库、智能体、技能
执行`docs/fagency.sql`，建表SQL后面就是insert into
...
- 智能体+事件确定若干主技能和若干辅助技能
  - 主技能数>=0，辅助技能数>=0
  - 主技能跟智能体一直全部加载，辅助技能按需加载
- 一个智能体会加入不同的流程，不论智能体在哪个流程，都会用一样的技能处理一样的事件
- 不同的流程可以有意义不同但同名的事件（尽量避免），这些事件必须由不同的智能体接收
- 一个技能只能处理一个确定的事件返回一个要么由所执行的函数确定的事件，要么从大模型返回的事件
    - 不需要执行函数的技能可以让大模型直接返回一个事件
    - 需要执行函数的技能，函数必须返回一个事件，这个事件可以不确定是哪个事件
- 一个技能可以被不同的智能体拥有
- 智能体执行技能，按照大模型返回的数据调用函数，函数返回数据和事件名
- 智能体是流程的节点。执行一次流程，一个智能体可能多次接收不同的事件，使用不同的技能处理
- 技能是流程的边，是原子化的，只做一件事。
    - 函数是技能的一部分
    - 技能可以有零个或者多个函数
- 事件是沿着技能边在各个智能体节点之间流转的数据
- 智能体的代码不需要关心当前正在处理的事件，也不需要关心由什么技能处理当前事件，只需要：
    ...
    - 这样做可以：
        ...
        - 由于智能体通过函数执行功能，不是直接执行命令，可以在函数增加边界情况处理预防安全性问题，比如：
            - 参数检查
            - 误删文件等
...
- 生成文档和代码、修改BUG需要多轮迭代，多次尝试和探索
    - 循环的终止条件
        - 每次模型调用都会返回一个停止原因（stop reason），决定接下来发生什么：
            ...
            - **max_tokens：**模型的响应因达到词元限制而被截断。这在当前循环中无法恢复，循环以错误终止。
...
> 物理上限
> 每个模型都有最大词元限制，目前主流模型通常在 128K 到 200K 之间。这个硬性边界的存在有两个根本原因：
...
> 即使拥有 1M词元的上下文窗口，Coding Agent 实际上只能有效利用其中的 **10-15%**。超过 20% 后，成本和性能都会急剧恶化。大多数 Agent 会在达到约 20% 时强制中断，而最佳实践是在 15% 之前就重启会话。
> **新发现的任务被丢弃**

...
- 与类的差别
  ...
  - 只有`mut func`声明的函数才能在函数体内修改成员变量
    - 如果这个函数还是结构体实现的接口的成员声明，接口的成员声明也得是`mut func`
⚠️ **重要**：尽量使用类，以下情况可以考虑使用结构体
    - 对象很小时 -（实例成员变量在四个以内，且都是简单类型或引用类型时）
 ...
 * 如果是函数声明，还可以有以下内容，如果是属性声明只能有@return和@throws，如果是变量、常量和类、结构体、接口、枚举声明只有声明的功能、目的和意义
...
public class TestClass{
    @TestCase
    public func testCase(): Unit {
        // unit test code
        @Assert(expression)//expression如果不是true测试用例会立即失败
        @Assert(exprectedExpression, expression)//如果左右两个值如果不相等测试用例会立即失败
        @Expect(expression)//expression如果不是true当前行会失败，但是会继续执行当前单元测试函数内的剩余测试
        @Expect(exprectedExpression, expression)//如果左右两个值不相等当前行会失败，但是会继续执行当前单元测试函数内的剩余测试
    }
    @Bench
    public func bench(): Unit {
        //性能测试代码
    }
}
```

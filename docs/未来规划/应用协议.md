...
r: Retain
message id: 消息ID
msgl: 消息长度的字节数，除当前字节以外往后还有多少字节表示消息体字节数；msgl后面四比特是bytes of length的最低位四比特，后续是bytes of length剩余比特位按大端序编码。如果消息体字节数<16，则msgl是0。
...
```
    ...
    |serial number -|//大端序编码进程号
    |ThreadIdAsInt64|//高4位是线程号除当前字节以外后面剩余字节数，低4位是线程号大端序低4位
    |ThreadIdAsInt64|//线程号大端序编码
    |-  Timestamp  -|//DateTime(t)编码
```

https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901027

消息体长度和编解码格式采用[编解码器.md](./编解码器.md)。其它特性尽量采用MQTT，以消息体字节数尽量少为第一优先级。
...
| CONSUMERESP             | 0         | QoS1      | QoS0      | ACK       |
| PINGREQ                 | 0         | 0         | 0         | ACK       |
| ELECT                   | VOTE      | APPROVE   | 0         | ACK       |
| ACK                     | 0         | 0         | 0         | 0         |
| UNREGISTER              | 0         | 0         | 0         | ACK       |
- **AUTH**: 
  
  - HASHDATA 为1时，消息体包含鉴权信息，为0时，消息体为空
  
- **SLAVEOF**: 
  - REG: 0注册从节点，1：获取从节点
  - REG是0的时候，一定会ACK；REG是1的时候会返回从节点，不需要ACK
  ```
...
- **PUBLISH**：
  ...
  -   RETAIN: 遗愿标记，表示消息格式错误、消费失败、发布确认失败时是否进入遗愿队列。
  
  -   QoS: 服务质量
      -   0：最多一次，发送之后不再关心后续
      -   1：最少一次，发送之后，指定时间内没有收到确认，重新发送。对端可能收到重复消息
      -   2：有且只有一次
  -   DUP：是否重复发送的消息
      -   0：首次发送消息
      -   1：重复发送的消息

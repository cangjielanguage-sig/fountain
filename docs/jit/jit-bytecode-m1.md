...
| T11 | **TO_STR 黄金值**：§11.9 全类型格式 ×（±0 / NaN / Inf / 极端 Decimal / 4 字节码点插值 / 负 Duration / Regex round-trip `/pattern/flags` / 混合 `+` 双向 × 全标签类型 / DateTime × TZ∈{UTC, 含夏令时非 UTC}） | 三引擎输出逐字节一致；DateTime 行**固定 TZ** 采集（基准 TZ=UTC 黄金 + 非 UTC 附加用例），其余与黄金文件全等 |

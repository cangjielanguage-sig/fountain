...
| I7 | 解释器语义是规范，JIT 是等价加速器：同一字节码 + 同一输入，解释器与两个 JIT 后端的结果、错误码、site 必须**逐位一致**（**v0.18.23 起**：错误报告文本——源码行与变量清单〔**v0.18.25 起含变量名**〕——同样逐位一致，§9.16） |
...
56   8    dbgBase   诊断转储区基址（非托管内存；0=禁用；桥在 ctx 准备时写入并清零首格，§9.16）
```
...
**句柄窗口语义**：`[hwBase, hwTop)` 本次调用可见。M1 中窗口**只增不减**；仅白名单**分配型 helper**（§6：H24–H29、H32–H41、H50–H51、H53–H54、H58–H61、H69、H70–H72）可登记新槽，且每次调用至多登记 1 个——其运算结果（不可变值 / 闭包句柄）。**变更型 helper（H43–H48）不登记新槽**——它们修改已存在的容器（非常量槽）。`hwTop == hwCap` 时宿主就地扩容句柄表（§9.6），不因容量失败；**有分配单元 `hwCap` 初值 = `HandleTable.initialCapacity`（默认 128，可在调用前调整）**；无分配单元 `hwCap=0`。
...
21   ERR_HOST_EXCEPTION      (宿主（仓颉侧）异常专用码——覆盖闭包/预定义与包装/构造适配器/成员访问器；cause 含完整仓颉栈、桥打印；M2 引入、v0.18.28 扩面)
22   ERR_ANY_CAST            (Any as 动态转型失败——盒内容与目标类型不符；site=any_as 调用点；M3〔verMajor 3〕引入；与 7 值转型分列)
23   ERR_CONCURRENT_MOD      (for-in 遍历 Map 期间修改同一 map——转码自 std 的 ConcurrentModificationException，对齐仓颉；§11.9)
99   ERR_HELPER_PANIC        (引擎侧未识别异常兜底；任何桥/内置 helper 内部异常最终都以此码浮现；宿主代码异常自 M2 起归 21)
```
...
| 范围 | 抛出类型 |
|---|---|
| 1–20、23 | `JitException(code, site, fnId, msg)`（msg 附源码行与变量（名/值/类型）报告，§9.16） |
| 21 | `JitException(code, site, fnId, msg)`（msg 附报告，§9.16）；另附 `cause` 原始异常全文——含**完整仓颉异常栈**（v0.18.28） |
| 22 | `JitException(code, site, fnId, msg)`（msg 附报告，§9.16；site = `any_as` 调用点——M3/verMajor 3） |
...
> **M2/M3 增补（21：v0.18.28 入注、v0.18.29 入表；22：v0.18.31 入表）**：**21 `ERR_HOST_EXCEPTION`** = **宿主（仓颉侧）异常专用码**（原 `ERR_HOST_CALLBACK` 扩展更名）——**预定义通道未捕获异常亦归此码**（原"已知语义分派（除零 → 1 等）+ 未知 99"废止）；`cause` 附异常全文（含**完整仓颉异常栈**）、桥打印完整栈。**22 `ERR_ANY_CAST`** = **Any `as` 动态转型失败**（M3/verMajor 3 引入——盒内容与目标类型不符；**与 7 值转型分列**〔m3 §12 第 6 项〕；site = `any_as` 调用点）。详见 `jit-bytecode-m2.md` §2 / `jit-bytecode-m3.md` §2。
...
- **寄存器名说明**：本节用 x86_64 记法（`rbp`/`rsp`/`r15`）。AArch64 对应 `x29`/`sp`/`x19`，且返回地址在 `x30` 需由 prologue 保存——完整映射见 §12.2、§12.3。帧布局本身**两后端共享**（`frameSize` 统一按 16 字节对齐——对齐依据见 §12.2）。
...
**ctx 内存机制（"不得移动"的兑现）**：ctx 必须位于**非 GC 托管内存**——桥经 C 互操作 `malloc/free` 分配，或对宿主 `Array<UInt8>` 用 std.core `acquireArrayRawData`/`releaseArrayRawData` 配对钉住（钉住窗口内 GC 不搬移；未配对释放属运行时错误）。ctx 内只有整数标量（errCode/site/fnId/hwBase/hwTop/flags/nargs），**唯一例外 = `dbgBase`（指向桥持有的非托管诊断缓冲区，§9.16）**——非 GC 内存、不含托管对象指针；ctx 无任何指向托管对象的指针（决策 ④），GC 无需扫描。
...
| 34 | `IMOD` | a%b（余数符号随被除数；`INT_MIN % -1 → 0`，不报溢出）；源语言 `%` 仅 `(Int64, Int64)` 定义（§11.9） | 2 |
| 35 | `INEG` | -a | 3 |
| 36 | `IABS` | \|a\| | 3 |
| 37 | `IMIN` | min(a,b) | — |
| 38 | `IMAX` | max(a,b) | — |
...
| 75 | `CALL_HELPER` | u16 hid, u8 argc | 调 helper，压结果；**汇编器自动尾随 CHECK_ERR**（hid：1–62、69–72 = 固定 helper §6；[0x0100,0xFFFF] = 单元私有预定义 helper §8.4，仅源入口产物） | helper 码 |
...
- **正则操作符（v0.11，§11.9）**：`re ~ s` / `re !~ s` → `REGEX_IS_MATCH`（`!~` 加 `NOT_B` 取反）；`re/s/r/g` → `REGEX_REPLACE_ALL`；`re/s/r/n` → `REGEX_REPLACE_NTH`；`re/s/r/n,m` → `REGEX_REPLACE_RANGE`（H72，v0.18.19）——三者均为 helper 直呼（`CALL_HELPER` H58/H59/H72，替换无专用 opcode）（示意中 `re` 为 Regex 表达式**占位符**——变量/字面量皆可；分隔符恒为单 `/`：无 flags 字面量代入后其闭合 `/` 与分隔 `/` 相邻呈 `//`，带 flags 字面量与变量均为单 `/`）；优先级高于加减、低于乘除，左结合；字面量后的紧贴 `/` 优先于注释识别。
...
5. **异常语义错位**——用户闭包的业务异常是正常程序流，映射为 99（实现缺陷信号）不合理（解法：新错误码 `21 ERR_HOST_EXCEPTION`（v0.18.28 扩展：全部宿主侧调用面） + 复用 cause 通道；异常对象回译属第三档）；
...
- **第三档**：高阶（闭包返回闭包——指**回调链内**函数类型签名传递，与 §14「逃逸闭包回桥」（字节码闭包 → 宿主）为**独立两轴**）+ 异常对象回译——完整双向互操作。**（v0.18.27：本档冻结——实现时机无限期推迟；冻结依据（替代路径先行/地基冲击）与重议条件见 `jit-bytecode-m2.md` §13-2。）**

判定：M1 的形态上限是 §8.4（编译期注册、运行期只传数据）；运行期闭包实参 = M2 CALL_HOST 的核心议题，按档渐进开放；**第三档已冻结——实现时机无限期推迟**（见上）。
...
| `BAIL` | k | 独立基本块 `写槽；errCode>0 时单次转储（§9.16）；eax=0; jmp .epilogue` | 每函数至少一个 |
...
public class JitException <: Exception {
    ...
    public let sourcePos: ?SourcePos // 由 site 经站点表查得（§9.16）；不可得 → None
    public let message: String       // 桥按 code + site 静态表组装 + 源码行与变量（名/值/类型）报告（§9.16）
    public let cause: ?String        // code=99 时附 helper 原始描述
}

public struct SourcePos {            // 源位置（站点表条目值；§11.6/§9.16）
    public let line: Int64           // 1 起
    public let column: Int64         // 1 起（Unicode 码点）
}
...
1. ctx.errCode = 0; ctx.site = -1; ctx.hwTop = ctx.hwBase; ctx.fnId = f.id; ctx.dbgBase = 诊断转储区（(2+maxNLocals)×8，首格清零；§9.16）
...
3. 按 ctx.errCode 分派（伪码）：
    ...
    >0   → 组装报告（源码行 + 变量转储〔名/类型经静态表解析〕，§9.16）→ throw JitException(code, site, ...)
...
| C 窗口分配 | 执行期 | 分配型 helper（§6：H24–H29、H32–H41、H50–H51、H53–H54、H58–H61、H69、H70–H72）追加槽并推进 `hwTop`（§9.6） | 窗口 `[hwBase, hwTop)` |
...
| helper 内部异常 | 已在 helper 边界转码（§6/§9.5）：**引擎侧 → 1–20/99；宿主侧代码 → 21**（v0.18.28） |

- `call` **绝不让非 `JitException`/非 `InterruptedException` 的底层异常逃逸**——桥内兜底转换为 99（§9.5），这是"捕获编译产物执行过程中的所有异常"的规范定义。
- **宿主处置约定**：1–20 按域错误处理（19 = 桥层契约错误；17 = 源语言 throw，回译为源语言异常语义；4/5 等可按业务取默认值；20 = 源程序试图写常量容器，属前端缺陷或需改用可变副本）；**99 = 实现缺陷信号，必须上报**（`cause` 即原始异常描述），不得按可恢复错误吞掉；**21 = 宿主（仓颉侧）代码异常**（`cause` 含完整仓颉栈）——按宿主异常语义处置；-2 按取消处理。反模式：`catch (e: Exception)` 泛兜并忽略——宿主侧的泛兜只会销毁桥已完成分类的信息（§9.5 在 helper 内的同类兜底是义务，因为那是"翻译"层；宿主是"消费"层）。
...
public class SourceCompileException <: Exception {
    ...
    public let line: Int64       // 源码行（1 起）
    public let column: Int64     // 列（1 起，Unicode 码点）
    public let lineText: String  // 错误所在源码行的原文（不含换行；§9.16）
...
- **hid 编码（冻结）**：固定 helper H01–H62、H69–H72 ↔ hid 1–62、69–72（v0.15：+H60/H61；v0.16：+H62；v0.16.1：H49/H62 重定义为迭代器——`map_iter_begin`/`map_iter_step`；v0.17：+H69 `str_to_regex`；v0.18：+H70/H71 `list_new`/`map_new`；v0.18.19：+H72 `regex_replace_range`）；**63–68、73–255 保留**（63–68 为 M2/M3 草案规划占用）；**user helpers ↔ hid [0x0100, 0xFFFF]**（slot = hid − 0x0100，上限 65280，受 `maxPredefined` 收紧）。hid 0 非法。
- **编译期名字解析（需求 4）**：body 中 `name(` 调用点按顺序解析——`recursive` → 自递归（§8.0）；命中注册表 → user helper（实参表达式**从左到右**求值，逐参按签名静态校验）；未命中 → `SourceCompileException(kind=UnregisteredFunction)`，消息含**名字、字节位置、行号与该行源码原文、已注册清单与修复提示**（§9.16）。非调用的未定义标识符 → 同类异常（kind=UndefinedIdentifier）。**编译期错误不占用 §2 运行期错误码**。
...
- **异常转码（§9.5 user 版；v0.18.28 修订）**：impl 抛出的任何异常由 trampoline 兜底——**统一转码 21 `ERR_HOST_EXCEPTION`**（仓颉侧异常专用码；原"已知语义分派（除零 → 1 等）→ 对应码 / 未知 → 99"一并废止——语义细分由宿主经 cause 自助），**cause 附异常全文（含完整仓颉异常栈）、桥同时打印完整栈**，绝不让宿主异常穿越机器码边界。**cause 通道为调用私有**：错误描述字符串由宿主侧按本次调用记录（随调用销毁，不跨调用/线程共享）——并发失败调用各自的 code/site/cause 互不串扰（§8.3）。
...
// → SourceCompileException(kind=UnregisteredFunction, position=…, line=1, column=…,
//    lineText="{ x: Int64 => unknown(x) }",
...
4. **site 精度**：只在**可能失败**的指令前写 site；纯算术指令不写（保持热路径 3–4 条指令）。错误报告中 site 精度为"最近一个可能失败点"；源位置查询与运行期变量报告见附则 16。
...
16. **源位置与诊断报告（v0.18.23 冻结；v0.18.25 增变量名）**：
   - **源位置模型**：`SourcePos = { line, column }`（1 起；列按 Unicode 码点）。`ctx.site` = **code 段绝对字节偏移**（`SET_SITE` 立即数；未设置=-1）；`site → SourcePos` 经**站点表**查询（取 `off ≤ site` 的最大条目；无命中 → `sourcePos=None`）。**站点表随产物走**：`.fbc` 的 site 段（§11.6 序列化、crc32 覆盖）× **JIT 产物（JitFunction）保留同一站点表**——机器码 `SET_SITE` 立即数即查询 key；**名字表（name 段，v0.18.25）与站点表并列随产物走**——JIT 产物保留同一名字表（§11.6）；"字节码与 JIT 产物都携带源码行号**与变量名**"由此兑现。
   - **编译期错误（含源码行文本）**：编译管线（源 → 字节码 → JIT 产物）任一步失败必须**明确报告**——源入口 `compile(source)` 统一抛 `SourceCompileException`（字段见 §8.4：kind / position / **line / column / lineText** / hint；`lineText` = 错误所在源码行的**原文**，即"错误信息包含源码行文本"的兑现；前端解析、名字解析、装配校验、降低均适用；其中"未实现特性"类装配拒绝（§7 校验 2/8/11、§10.3 校验 3）kind 记 `NotImpl`——即 §2 错误码 18 的编译期呈现）。分发入口 `compile(unit)`（只有 `.fbc`、无源文本）：加载/装配失败抛 `JitException`（code ∈ {14,18}），msg 含**行号**（站点表可查时）、不含文本——`.fbc` 的调试元数据为**符号级**（站点表 + 变量名表，§11.6），不携带源文本。
   - **运行期报告（源码行 + 各变量名、值与类型）**：错误码 1–20、23、99（**M2 起另含 21**）的 `JitException` 组装时**必须**附：① 源码行（`sourcePos`）；② **各局部变量的名字、值与类型**（名字 = 源码级命名变量，v0.18.25；临时/隐藏槽无名）。为此 fail-fast 收敛时执行**单次转储**：凡 `errCode > 0` 的收敛点（`BAIL` / `CHECK_ERR → bail`；-1/-2 不转储），若 `ctx.dbgBase ≠ 0` 且缓冲首格为 0（未填；桥清零）→ 依次写 `[dbgBase]=fnId`、`[dbgBase+8]=nLocals`、拷 `locals[0..nLocals)` 原样 cell——**第一个收敛点获胜**（= 最深失败帧；外层传播帧因哨兵跳过）。桥在抛出前组装报告：行号取自站点表；变量取自转储——**名字查 name 段**（§11.6，按 `fnId` + 槽位序号；**无条目 → 省略名字**）、类型来自该函数局部类型表（§11.7），值呈现：值型（0–3）按 §11.9 文本化（Unit 记 `Unit`）、句柄型（4–12）记 `<类型名>#<idx>`（0 → `<类型名>#0`；**不取对象**，避免错误路径二次失败）。缓冲区 = `(2 + maxNLocals) × 8`（max 取目录全域；桥分配/清零，§8.2 第 1 步）。报告文本模板（冻结，三引擎逐位比对）：`（code=N，site=OFF）` ⏎ `  源码行 L:C · fnId=F` ⏎ `  变量: [0] a: Int64 = 42; [1] s: String#7; [2] Int64 = 3; …`（**条目格式：有名字 → `名字: 类型…`；无名字 → 直接类型**）。**边界（M1 冻结）**：以**槽位序号**为键、名字经 name 段解析（v0.18.25；**旧单元 verMinor ≤ 13 无名字表 → 全部按无名形态**）；不含操作数栈；`site=-1` → 无行号、无变量；`dbgBase=0`（防御分支）→ 跳过转储、报告仅含行号。
...
0    4    magic       "FBC1"
4    2    verMajor    1
6    2    verMinor    14（递增原因＝新增 opcode / 新增 helper / 类型表格式变更 / CALL_HELPER hid 值域扩展 / 站点段序列化定义 / **变量名段序列化定义**；历史：v0.4=1、v0.5=2、v0.6=3、v0.7=4、v0.8=5、v0.10=6、v0.11=7、v0.15=8、v0.16=9、v0.17=10、v0.18=11、v0.18.19=12、v0.18.23=13、**v0.18.25=14**；加载器接受 ≤ 自身版本的 verMinor）
8    4    flags       bit0=UTF-8 语义(恒 1)  bit1=需要 JIT(必须为 0)  bit2=使用值运算/分配/变更类指令（helper 路由：A0–A8 / 0x9A–0x9E / 0xAB–AF / 0xB0–0xBF 时置 1）
12   4    hdrSize     36（v0.18.25 起；旧单元 32）——magic 之后的头长度，payload = 4 + hdrSize（v1 预留）
16   4    codeLen
20   4    constLen
24   4    siteLen     站点表（偏移 → 源位置）长度
28   4    entryFn     入口函数 id
32   4    nameLen     变量名表（name 段）长度（v0.18.25 新增；≥ 4）
36   4    crc32       code + const + site + name
40   ...  code / const / site / name 四段依次存放
```
...
- 站点表（site table）是**平台无关的偏移 → 源位置映射**，三引擎的错误报告都从这里取源位置；**JIT 产物（JitFunction）保留同一站点表**——机器码 `SET_SITE` 立即数即查询 key（§9.16）。**名字表（name 段，v0.18.25）与站点表并列随产物走**——JIT 产物保留同一名字表，运行期报告据此解析变量名（§9.16）。
- **site 段序列化（v0.18.23 冻结）**：`SiteTable { count: u32, entries: [SiteEntry] }`，`SiteEntry = (off: u32, line: u32, column: u32)`（恒 12 B）——`off` = site 生效位置在 **code 段的绝对字节偏移**（含函数 `codeOff` 基准），**升序、唯一**；`line` / `column` 1 起（列按 Unicode 码点，与 §8.2 `SourcePos` 同定义）。
- **覆盖率与校验**：条目集合必须覆盖**全部 `SET_SITE` 位置**（前端义务）；汇编器校验每条 `SET_SITE` 立即数命中条目，否则拒绝；加载器校验 `count > 0` 时严格升序、`off < codeLen`、`line ≥ 1`、`column ≥ 1`；纳入 `crc32`（容器头 `siteLen`，上文）。查询 = 取 `off ≤ site` 的最大条目；无命中 → `sourcePos=None`（§9.16）。
- **头布局兼容（v0.18.25）**：`nameLen` 插入 `crc32` 前——`crc32` 偏移 32→36、payload 起始 36→40；其余字段偏移不变。加载器按 `verMinor` 分支：≤ 13 = 旧布局（无 `nameLen`、payload 36、无 name 段）；≥ 14 = 新布局（payload = 4 + `hdrSize` = 40）；「加载器接受 ≤ 自身版本的 verMinor」规则不变。
- **name 段序列化（v0.18.25 冻结）**：`NameTable { count: u32, entries: [NameEntry], strBytes: [u8] }`，`NameEntry = (fnId: u32, slot: u32, strOff: u32, strLen: u32)`（恒 16 B；`strOff` 相对 `strBytes` 区起点，`strBytes` 紧接 `entries` 之后）；`(fnId, slot)` 升序、键唯一；名字 = 源码标识符的 UTF-8 字节；`nameLen = 4 + 16 × count + strBytes 长度`（段恒存在，`count` 可为 0）。**前端义务**：为全部**源码级命名槽位**（参数、`for` 迭代变量、赋值/声明目标）登记；编译器引入的隐藏槽（多重赋值暂存、容器/映射迭代的索引·长度·迭代器状态、`recursive` 值化适配器函数、闭包帧 `locals[0]` 被调句柄）**不登记**。**加载器校验**：`fnId` 命中目录、`slot <` 该函数 `nLocals`、`(fnId, slot)` 严格升序唯一、`strLen ≥ 1`、`strOff + strLen ≤ strBytes 长度`、UTF-8 合法；纳入 `crc32`（容器头 `nameLen`，上文）。查询 = 按（转储 `fnId` + 槽位序号）命中；未命中 → 报告省略名字（§9.16）。
...
- **变量名表（§11.6 name 段，v0.18.25）与类型表相互独立**：类型表**每槽必有**（覆盖全部 `nLocals`），名字表**稀疏**（仅源码级命名槽）——运行期报告对无条目槽**省略名字**（§9.16）。
...
- **`%`（取余，v0.18.24 明确；冻结）**：源语言 `%` **仅对 `(Int64, Int64)` 定义**——降低 `IMOD`（0x34）。语义对齐仓颉：`a % b := a - b * (a / b)`（`/` 向零截断 ⇒ **余数符号随被除数**：`4 % -3 = 1`、`-4 % 3 = -1`）；`b = 0 → 2`；`INT_MIN % -1 → 0`（数学余数恒 0，**不报 3**，§12.4）。**`%` 不参与提升**：任一操作数非 Int64（含 F64/Decimal/混合类型）→ 编译错误（与仓颉"`%` 操作数只支持整数类型"一致）；优先级 = 乘法级（`* / %`，左结合——§11.9 正则条已引用）。
...
| `re/s/r/n,m` | **区间替换**：自第 n 个匹配（含）起连续替换 **m 个**（n 为非负十进制字面量、从 0 计；m 为 ≥ 1 的十进制字面量（**`m ≤ 0` → 编译错误**）；剩余不足 m → 替换到末尾、不报错；H72，v0.18.19） | `REGEX_REPLACE_RANGE` |

（表中 `re` 为 Regex 类型表达式**占位符**（变量或字面量），分隔 `/` 恒为单个——代入后按实际拼写：无 flags 字面量 = 其闭合 `/` 与分隔 `/` 相邻（`/\s//s/r/g`）；带 flags 字面量 = `/[a-z]/i/s/r/g`；变量 = `re/s/r/g`。详见下条「替换段词法」。**替换类「降低」名是 helper 直呼的记法**（替换**无专用 opcode**）：`REGEX_REPLACE_ALL` = `CALL_HELPER` H58、`REGEX_REPLACE_NTH` = `CALL_HELPER` H59、`REGEX_REPLACE_RANGE` = `CALL_HELPER` H72——按各自签名传参并自动 `CHECK_ERR`。）
...
- **替换段词法（冻结；v0.16.3 澄清 `/` 个数，v0.16.4 示意记法修正）**：替换段由**单个 `/`** 开启（示意记法统一以单 `/` 书写：`re/s/r/g`——`re` 为 Regex 类型表达式**占位符**（变量/字面量皆可）；无 flags 字面量代入后其**闭合符**自然与**分隔符**相邻呈 `//`，并非分隔符翻倍），LHS 须为 Regex 类型表达式（正则字面量常量或 Regex 局部变量，§11.7；非 Regex 左操作数的 `/` 仍是除法——Regex 无除法运算，按类型无歧义）。开启符的书写形态随 LHS 而定：
  ...
  替换段依次为：目标串表达式、`/`、replacement 表达式、可选 `/` + 修饰符——修饰符取三式之一：`g`（全部）、十进制整数 `n`（单个，从 0 计）、`n,m`（**区间**：两个**无空白**十进制字面量，`n ≥ 0`、`m ≥ 1`；**`m ≤ 0` 或任一超出 Int64 域 → 编译错误**）；修饰符段以 `-` 开头 → 词法错误；`g` 与数字不得混写（如 `g,2` → 词法错误）。
- **replacement 语义**：**字面文本**替换（`$` 不展开——捕获组引用属 M2）；空 replacement = 删除匹配部分，合法。
- **边界（冻结）**：① pattern 为空 → 加载期拒绝（§3.3）——非空 pattern 保证每次匹配至少 1 字节，替换扫描天然推进、**无零宽匹配死循环**；② `n` 超出匹配次数或整体无匹配 → 替换返回**原串**（不报错），`~` 返回 false、`!~` 返回 true；③ `n = 0` 即第一个匹配；④ flags（`i m u`）对替换同样生效（同一 Regex 常量）；⑤ 句柄 null → 6（既有 fetch 语义）；⑥ 替换结果为新 String（分配型——I8/重放安全：重放重建等值串）；⑦ **`n,m` 区间形式**：只替换序号 ∈ `[n, n+m)` 的匹配（单遍扫描完成，其余匹配原样）——**剩余不足 m → 替换到最后一个匹配**（m 可超过剩余数，不报错）；n ≥ 匹配总数或整体无匹配 → 原串（同 ②）；`n=0,m=1` ≡ `re/s/r/0`；`n=0` 且 m ≥ 总数 ≡ `/g`。
...
```cangjie
import fountain::f_base.{OS, Arch}         // 平台判定（f_base 模块）

let engine = if (opts.jit == JitMode.Off) {
    Engine.Interp
} else {                                   // Auto / Force
    if (OS.current.isLinux && (Arch.current.isX86_64 || Arch.current.isAarch64)) {
        Engine.Jit        // Auto 另以字节码指纹为缓存键
    } else {
        diag("jit unavailable on ${OS.current}/${Arch.current}; fallback to interp")
        Engine.Interp
    }
}
```
...
- 两平台栈均要求 **16 字节对齐**，故 §3.1 的 `frameSize` 统一按 16 取整，两后端共用同一计算式。
  - **对齐依据（v0.18.22 成文）**：16 不是"取大"，而是两套宿主 ABI **硬性要求的交集**——① **AArch64（AAPCS64）**：SP 在公共接口处必须 quad-word 对齐（`bl` 不压栈 ⇒ "恒 16"，无条件）；② **x86_64（SysV）**：控制转移到函数入口时 `rsp + 8` 是 16 的倍数（等价 `call` 前 `rsp % 16 == 0`，锚点在**调用边界**）。对端（宿主 trampoline、仓颉编译器/LLVM 生成的 helper 代码——典型如 `movaps` 寄存器溢出槽、glibc 变参处理）**合法假设 16**；失配不是"慢一点"，而是随机 SIGSEGV / 直接违反 AAPCS64。
  - **算术（"取整到 16"为何恰好够）**：x86 入口 `rsp ≡ 8 (mod 16)` → `push rbp` 后 `≡ 0` → `sub rsp, FRAME`；aarch64 `stp x29,x30,[sp,#-16]!` 后 `≡ 0` → `sub sp, sp, #FRAME`——两边同构 ⇒ **`FRAME ≡ 0 (mod 16)` ⇔ 每个调用点对齐**。按 8 取整则一半帧失配（x86 崩 / aarch64 违约），且被迫为两后端各写 padding，破坏"共用同一公式"；按 32/64 则纯浪费（SysV 的 32/64 仅是大参数区可选上界）。
  - **层级区分**：cell = 8 字节、槽 8 字节对齐（§11.2）是**数据布局**要求；栈 16 字节是**调用契约**要求（§6 trampoline ABI 对端可信任的保证）。代价 ≤ 8 字节/帧。
...
| T12 | **混合类型差分**：§11.9 提升矩阵全组合（3 类型对称对 × 四则与比较 × 提升方向，含 2^53 边界、`0.1` 类浮点、除零、空区间判定）；**`%` 取余（v0.18.24）**（仅 `(Int64, Int64)`：符号四象限 `±4 % ±3`、`b = ±1`、`a = 0`、`INT_MIN % -1 → 0`、`b = 0 → 2`；常量折叠；**非 Int64/混合 → 编译错误**）；**小数字面值精确构造**（`0.1` 字面值 → Decimal("0.1")，与 `0.1f64` 位模式可区分；指数形式 `3e2`/`1e-3` 的 scale 正确）；**显式窄化与超 Int64 字面值**（F64→I64 越界 → 13、DEC→I64 越界 → 9；超 Int64 整数字面值 → Decimal scale=0；>34 位有效数字**精确保留**——任意精度） | 三引擎结果与错误码逐位一致，与提升规则推导值全等；字面值构造、窄化失败码与 §11.9 规则全等 |
...
| T20 | **字符串切片/替换与正则（v0.6）**：STR_SUB（越界 → 4、`a==b` → 空串、`s[a..=b]` 前端折算等价性）；STR_REPLACE（空模式 → 14、多匹配全替换、替换串含模式子串）；正则字面量 ×（i/m/u 全组合、**任意书写顺序、重复幂等** × is_match/find/无匹配 → 0），同 (pattern, flags) 单元内同 ID、**编译次数 == 常量数**（一次编译断言）；非法 pattern → 加载期 `JitException(14)`；**v0.17 增补**：`f_regex` 字符串构造（`regexStr.regex()` → `STR_TO_REGEX`：合法串往返、空串/非法 pattern → 14、恒无 flags；分配型——窗口登记 1 槽，每次调用新对象） | 三引擎结果/错误码/site 逐位一致；复用与去重断言成立；正则语义与 std.regex 一致（同版本 std 下跨平台确定） |
...
| T22 | **源入口格式与自递归（v0.7）**：lambda 源入口全形态（无参/多参/容器参数 List/Map/`=>` 缺失 → 前端拒绝/具名与默认参数 → 拒绝；**Regex 参数自 v0.17 开放——移出拒绝面**）；返回类型推导 × `call<T>` 校验（T 与尾表达式不符 → 19；分发入口按 `FuncEntry.retType` 校验）；`recursive` 深度递归（参数逐位一致、`fnId` 归因随递归层修正、中等深度 × 三引擎、含变更型的递归函数受 §9.15 约束） | 非法源码前端拒绝；`call<T>` 结果类型识别全部正确；递归结果/错误码/site 三引擎逐位一致；retType 非法值被加载器拒绝 |
| T23 | **Compiler 门面与预定义函数（v0.8）**：注册/重名/超限/注销；名字解析（未注册调用 → SourceCompileException 含名字/位置/已注册清单；未定义标识符；形参遮蔽调用 → 拒绝）；user helper 调用桥（参数/返回 marshal × 全 LocalType 组合、impl 异常 → 21 + cause（含完整栈；v0.18.28）、返回句柄登记/逃逸/自动回收）；含预定义调用 → flags.bit0 + deopt 禁令；分发装载含 user hid 的 `.fbc` → 加载期拒绝；并发注册 + 编译 | 四需求全路径正确；异常信息含名字/位置/已注册清单；三引擎逐位一致；快照语义（注册变更不影响已编译产物）；分发拒绝正确 |
...
| T26 | **正则操作符（v0.11）**：`~` / `!~` ×（匹配/不匹配/无匹配/flags i-m-u 组合/空 replacement）；替换 ×（`g` 全局、省略修饰符 = 第 0 个、`/n` 各位次含越位 → 原串、**`/n,m` 区间**（含第 n 个、剩余不足 m → 到末尾、m 超剩余数、n 越位 → 原串、`n=0,m=1` ≡ `/0`、`n=0`+大 m ≡ `/g`）、多匹配连续扫描、replacement 含正则元字符与 `$` 字面保留、空 pattern 加载期拒绝；**三书写形态**——无 flags 字面量 `/\s//s/r/g`（闭合符+开启符相邻呈 `//`）、带 flags 字面量 `/[a-z]/i/s/r/g`（单 `/`）、Regex 变量 `re/s/r/g` 与 `re/s/r/1`（单 `/`），三形态同语义同产物）；词法（`!~` 单 token、替换段紧贴 `/` 优先于注释识别、**变量后 `//` 按注释识别、不开启替换段**、`-` 修饰符 → 词法错误、`n,m` 字面量约束（无空白、`m ≥ 1`；**`m ≤ 0` → 编译错误**）与 `g`/数字不混写）；优先级/结合性（高于加减低于乘除、左结合 × 与算术混排） | 四形式语义与 §11.9 全等；三书写形态（无 flags `//` / 带 flags 单 `/` / 变量单 `/`）产物逐字节等价；三引擎结果/错误码/site 逐位一致；分配型语义（重放等值）；优先级与结合性按文法全等 |
...
| T50 | **源位置与诊断报告（v0.18.23；v0.18.25 增变量名）**：编译期错误 ×（前端各 kind——`SourceCompileException` 含 line/column/`lineText`＝该行原文）；汇编期拒绝 ×（未实现指令 / §7 校验 1–11 → 源位置随报）；site 段 ×（覆盖率＝全部 `SET_SITE`、升序与边界校验、坏表加载期拒绝、crc32 覆盖）；**name 段 ×（键合法性〔fnId 命中目录、slot < 该函数 nLocals〕、升序唯一、UTF-8、`strLen ≥ 1`、坏表加载期拒绝、crc32 覆盖；旧单元〔verMinor ≤ 13〕无段兼容）**；运行期报告 ×（错误码 1–20/23/99〔M2 起含 21〕 ×（源码行 + 变量清单**含名字**（参数/迭代变量/赋值目标；临时/隐藏槽省略名字）：值型全 LocalType 0–3、句柄型 `#idx`、null 槽、最深帧获胜、外层传播不覆盖、DEOPT 不转储）；解释器路径同报告；`dbgBase=0` 防御分支仅行号）；分发入口（无源文本 → 行号不含文本） | 编译错误文本 == 对应源码行原文；行号与变量清单（含名字）**三引擎逐位一致**（I7 扩展）；`sourcePos` 空值路径正确 |
| T51 | **容器字面值创建与返回矩阵（v0.18.26）**：创建 × 两态——空字面值 `[]`/`{}`（运行期构造 MAKE_LIST/MAKE_MAP、每次求值新实例）vs 非空字面值（常量：单元内共享、DAG 复用、ID 跨调用稳定）；上下文类型来源（局部注解 / 实参签名；无上下文 → 编译错误）；求值上下文（直序 / 分支内 / 循环内——空者逐次新、非空者恒同一常量）；返回 ×（空/非空 × 字面值变量返回〔v0.18.1〕＋ retType=10/11 逃逸 `get` 解包——空：每次调用新对象、调用内 ID 稳定、跨调用不保证；非空：共享只读对象、写 → 20、宿主只读契约、D 路径对路径 A no-op）；传递 ×（字面值作实参 / 嵌套闭包捕获 / CALL_CLOSURE 参数与返回）；窗口账目（空构造每次登记 1 槽 / 非空常量 0 槽）+ deopt 重放（空等值重建、非空不重建）；对照：闭包字面值变量返回 → 编译错误。与 T48 对拍（T48 侧重自建/变更/降序产物；本项为字面值创建·身份·返回全矩阵） | 三引擎结果/错误码/site 逐位一致；身份语义（新实例 vs 共享常量、调用内/跨调用）与 §3.3/§8.1 规则 4 全等；无泄漏（T27 模式） |
| T52 | **全类型传参与返回矩阵（v0.18.26）**：LocalType 0–12（Int64 / Float64 / Bool / Unit / String / Decimal / DateTime / Duration / Range / Regex / List / Map / Closure）× 通道（源入口实参〔§8.2.1 表——Closure 不在列〕、`call<T>` 返回解包〔§8.2.2〕、CALL_FUNC 链内传参与返回、CALL_CLOSURE 传参与返回、预定义 user helper 传参与返回）逐格覆盖（按各通道合法类型集取格；源入口无 Closure / Unit 实参行）——每格正常往返值逐位一致；多参混排（值型＋句柄型交错、槽序/栈序）；每型边界值（Int64 极值 / NaN·±Inf / 空串与 4 字节码点 / 空容器 / DateTime·Duration 值域端点 / 空区间 / Regex 无匹配）；错误面（null、参数长度不符、元素类型不符、`T` 与 retType 不符 → 19；Closure 经 `call<T>` 逃逸 → 19 或前端拒绝；retType 非法值 → 加载期拒绝）；跨通道同值对拍（同一单元、同一输入经各通道往返结果一致；各通道差分/错误面基线见 T15/T23/T25） | 类型 × 通道矩阵零漏项（计数断言）；三引擎结果/错误码/site 逐位一致；Marshal/解包与 §8.2.1/§8.2.2 全等 |
| T53 | **容器读写操作符与变更方法矩阵（v0.18.26）**：`[]` 读——List `l[i]`（首/中/末、越界 → 4）、Map `m[k]`（命中、缺键 → 5；GET_KEY_OR/HAS_KEY〔0x93/0x94，ISA 级直测〕对照）；`[]` 写——`l[i] = v`（LIST_SET：覆盖写、越界 → 4）、`m[k] = v`（MAP_PUT：新增键与覆盖键）；方法——LIST_APPEND（尾插）、LIST_INSERT（定点插、顺序保持、i 越界 → 4）、LIST_REMOVE（**返回被删元素**、越界 → 4）、MAP_REMOVE（**返回 0/1**、本无此键不报 5）；组合序列（增删查改交替＋循环内批量：顺序、长度、宿主侧可见的容器终态一致）；错误面抽查（常量目标 → 20、null → 6、tag 不符 → 8；全组合差分基线见 T18）；窗口账目（变更 0 槽）+ 含变更函数 deopt 禁令 | 语义与 std 等价物逐位一致（含 LIST_REMOVE 返回值 / MAP_REMOVE 0/1 语义）；三引擎结果/错误码/site 逐位一致；无泄漏 |

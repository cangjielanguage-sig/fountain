...
> **M2 增补（v0.18.28）**：**21 `ERR_HOST_EXCEPTION`** = **宿主（仓颉侧）异常专用码**（原 `ERR_HOST_CALLBACK` 扩展更名）——**预定义通道未捕获异常亦归此码**（原"已知语义分派（除零 → 1 等）+ 未知 99"废止）；`cause` 附异常全文（含**完整仓颉异常栈**）、桥打印完整栈。详见 `jit-bytecode-m2.md` §2。

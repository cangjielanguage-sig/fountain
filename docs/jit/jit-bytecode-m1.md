...
| I7 | 解释器语义是规范，JIT 是等价加速器：同一字节码 + 同一输入，解释器与两个 JIT 后端的结果、错误码、site 必须**逐位一致**（**v0.18.23 起**：错误报告文本——源码行与变量清单——同样逐位一致，§9.16） |
...
| 范围 | 抛出类型 |
|---|---|
| 1–20、23 | `JitException(code, site, fnId, msg)`（msg 附源码行与变量报告，§9.16） |
...
public class JitException <: Exception {
    ...
    public let message: String       // 桥按 code + site 静态表组装 + 源码行与变量报告（§9.16）
...
3. 按 ctx.errCode 分派（伪码）：
    ...
    >0   → 组装报告（源码行 + 变量转储，§9.16）→ throw JitException(code, site, ...)
...
16. **源位置与诊断报告（v0.18.23 冻结）**：
   - **源位置模型**：`SourcePos = { line, column }`（1 起；列按 Unicode 码点）。`ctx.site` = **code 段绝对字节偏移**（`SET_SITE` 立即数；未设置=-1）；`site → SourcePos` 经**站点表**查询（取 `off ≤ site` 的最大条目；无命中 → `sourcePos=None`）。**站点表随产物走**：`.fbc` 的 site 段（§11.6 序列化、crc32 覆盖）× **JIT 产物（JitFunction）保留同一站点表**——机器码 `SET_SITE` 立即数即查询 key；"字节码与 JIT 产物都携带源码行号"由此兑现。
   - **编译期错误（含源码行文本）**：编译管线（源 → 字节码 → JIT 产物）任一步失败必须**明确报告**——源入口 `compile(source)` 统一抛 `SourceCompileException`（字段见 §8.4：kind / position / **line / column / lineText** / hint；`lineText` = 错误所在源码行的**原文**，即"错误信息包含源码行文本"的兑现；前端解析、名字解析、装配校验、降低均适用；其中"未实现特性"类装配拒绝（§7 校验 2/8/11、§10.3 校验 3）kind 记 `NotImpl`——即 §2 错误码 18 的编译期呈现）。分发入口 `compile(unit)`（只有 `.fbc`、无源文本）：加载/装配失败抛 `JitException`（code ∈ {14,18}），msg 含**行号**（站点表可查时）、不含文本——`.fbc` 不携带源文本（如需，属后续调试段增量）。
   - **运行期报告（源码行 + 各变量的值与类型）**：错误码 1–20、23、99 的 `JitException` 组装时**必须**附：① 源码行（`sourcePos`）；② **各局部变量的值与类型**。为此 fail-fast 收敛时执行**单次转储**：凡 `errCode > 0` 的收敛点（`BAIL` / `CHECK_ERR → bail`；-1/-2 不转储），若 `ctx.dbgBase ≠ 0` 且缓冲首格为 0（未填；桥清零）→ 依次写 `[dbgBase]=fnId`、`[dbgBase+8]=nLocals`、拷 `locals[0..nLocals)` 原样 cell——**第一个收敛点获胜**（= 最深失败帧；外层传播帧因哨兵跳过）。桥在抛出前组装报告：行号取自站点表；变量取自转储——类型来自该函数局部类型表（§11.7），值呈现：值型（0–3）按 §11.9 文本化（Unit 记 `Unit`）、句柄型（4–12）记 `<类型名>#<idx>`（0 → `<类型名>#0`；**不取对象**，避免错误路径二次失败）。缓冲区 = `(2 + maxNLocals) × 8`（max 取目录全域；桥分配/清零，§8.2 第 1 步）。报告文本模板（冻结，三引擎逐位比对）：`（code=N，site=OFF）` ⏎ `  源码行 L:C · fnId=F` ⏎ `  变量: [0] Int64 = 42; [1] String#7; …`。**边界（M1 冻结）**：以**槽位序号**为键（源变量名/名字表属后续增量）；不含操作数栈；`site=-1` → 无行号、无变量；`dbgBase=0`（防御分支）→ 跳过转储、报告仅含行号。
...
0    4    magic       "FBC1"
4    2    verMajor    1
6    2    verMinor    13（递增原因＝新增 opcode / 新增 helper / 类型表格式变更 / CALL_HELPER hid 值域扩展 / 站点段序列化定义；历史：v0.4=1、v0.5=2、v0.6=3、v0.7=4、v0.8=5、v0.10=6、v0.11=7、v0.15=8、v0.16=9、v0.17=10、v0.18=11、v0.18.19=12、v0.18.23=13；加载器接受 ≤ 自身版本的 verMinor）
...
- 站点表（site table）是**平台无关的偏移 → 源位置映射**，三引擎的错误报告都从这里取源位置；**JIT 产物（JitFunction）保留同一站点表**——机器码 `SET_SITE` 立即数即查询 key（§9.16）。
...
| T50 | **源位置与诊断报告（v0.18.23）**：编译期错误 ×（前端各 kind——`SourceCompileException` 含 line/column/`lineText`＝该行原文）；汇编期拒绝 ×（未实现指令 / §7 校验 1–11 → 源位置随报）；site 段 ×（覆盖率＝全部 `SET_SITE`、升序与边界校验、坏表加载期拒绝、crc32 覆盖）；运行期报告 ×（错误码 1–20/23/99 ×（源码行 + 变量清单：值型全 LocalType 0–3、句柄型 `#idx`、null 槽、最深帧获胜、外层传播不覆盖、DEOPT 不转储）；解释器路径同报告；`dbgBase=0` 防御分支仅行号）；分发入口（无源文本 → 行号不含文本） | 编译错误文本 == 对应源码行原文；行号与变量清单**三引擎逐位一致**（I7 扩展）；`sourcePos` 空值路径正确 |

...
| TO_STR / 插值 / 混合 String 加法 / 数值转换依赖平台 locale 或宿主格式化库 | 违反 P3；全部格式已由 §11.9 冻结，实现必须内嵌同一算法 |
...
| T11 | **TO_STR 黄金值**：§11.9 全类型格式 ×（±0 / NaN / Inf / 极端 Decimal / 4 字节码点插值 / 负 Duration / Regex round-trip `/pattern/flags` / 混合 `+` 双向 × 全标签类型） | 三引擎输出逐字节一致，与黄金文件全等 |

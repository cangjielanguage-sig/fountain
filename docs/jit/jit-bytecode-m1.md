...
| T50 | **源位置与诊断报告（v0.18.23；v0.18.25 增变量名）**：编译期错误 ×（前端各 kind——`SourceCompileException` 含 line/column/`lineText`＝该行原文）；汇编期拒绝 ×（未实现指令 / §7 校验 1–11 → 源位置随报）；site 段 ×（覆盖率＝全部 `SET_SITE`、升序与边界校验、坏表加载期拒绝、crc32 覆盖）；**name 段 ×（键合法性〔fnId 命中目录、slot < 该函数 nLocals〕、升序唯一、UTF-8、`strLen ≥ 1`、坏表加载期拒绝、crc32 覆盖；旧单元〔verMinor ≤ 13〕无段兼容）**；运行期报告 ×（错误码 1–20/23/99 ×（源码行 + 变量清单**含名字**（参数/迭代变量/赋值目标；临时/隐藏槽省略名字）：值型全 LocalType 0–3、句柄型 `#idx`、null 槽、最深帧获胜、外层传播不覆盖、DEOPT 不转储）；解释器路径同报告；`dbgBase=0` 防御分支仅行号）；分发入口（无源文本 → 行号不含文本） | 编译错误文本 == 对应源码行原文；行号与变量清单（含名字）**三引擎逐位一致**（I7 扩展）；`sourcePos` 空值路径正确 |

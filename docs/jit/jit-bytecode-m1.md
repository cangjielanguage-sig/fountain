...
- 闭包变量可赋值给同签名变量、向下传给 CALL_FUNC 被调函数、被嵌套闭包按值捕获（句柄快照）、在 CALL_FUNC 链内返回（retType=12）；**不可逃逸**（跨桥/存容器——§8.6 边界；「链内返回」不属逃逸）。
...
**M2 边界（v0.10 修订）**：仍不做——按引用捕获（boxed upvalue）、闭包逃逸（跨桥/存容器）、闭包签名内嵌套闭包类型（非高阶）、运行期动态闭包组合。first-class 的局部形态（变量持有、调用、向下传参、链内返回）自 v0.10 支持（§8.6）。
...
**正则操作符（v0.11，冻结；v0.16.3 澄清 LHS 形态）**：LHS 恒为 Regex 类型（正则字面量常量或 Regex 局部变量，§11.7 类型集含 Regex；Regex 不可作参数/返回，§3.3）；RHS 为 String 类型表达式；类型不符 → 编译错误。

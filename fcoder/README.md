...
- 技能名称必须唯一
- 一个事件会被不同的智能体接收
- 一个智能体可以接收多个事件，拥有多个技能；也可以返回多个事件
- 不同的智能体会使用不同的技能处理同一个事件
- 智能体+事件确定一个技能
- 一个智能体会加入不同的流程，不论智能体在哪个流程，都会用一样的技能处理一样的事件
- 不同的流程可以有意义不同但同名的事件（尽量避免），这些事件必须由不同的智能体接收
- 一个技能只能处理一个确定的事件返回一个确定的事件
- 一个技能可以被不同的智能体拥有
- 智能体是流程的节点。执行一次流程，一个智能体可能多次接收不同的事件，使用使用不同的技能处理
- 技能是流程的边，是原子化的，只做一件事。
- 智能体的代码不需要关心当前正在处理的事件，也不需要关心由什么技能处理当前事件，只需要：
    1. 按照事件查询技能
    2. 把事件和技能包装为大模型访问参数
    3. 按照大模型返回的JSON去执行函数
    4. 按照函数返回构造事件、返回事件
...
> **尽量使用中文**
> 智谱一个token大约是0.75个英文单词或者1.5个汉字，其它大模型也差不多，有0.75个英文词或0.75个汉字的，但是大部分都是一个token对应的汉字更多一些。从信息密度考虑相同的token数显然汉字能表达更多意义，随着token数增长这个趋势越明显。
> 从节省token的角度考虑，尽量使用中文。